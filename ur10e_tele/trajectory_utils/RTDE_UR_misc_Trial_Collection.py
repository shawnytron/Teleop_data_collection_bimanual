...
def horizon_reset(env,controller, control_hz, right_controller):
    env.close()
...
def collect_trajectory(
    ...
    controller=None,
    eps_horizon=None,
    save_filepath="/home/demoaccount/Data/demoaccount2/teleop_data_collect/outputs",
    task = "",
    save_data = False, #bool flag for if it i just a test run or collection run
    save_hz = 5,
    # start_index=0,
    # metadata=None,
    save_images=False,
    ...
    - collects data and save traj if save_data is true
    - eps_horizon =  number of epsoide to collected with in a trial session, program will exit if horizon is reached or esc is pressed on computer
    - A,B OR X,Y on controller saves epsiode -> moves robot back to default position -> 
    prepare to start collecting data again,saving under new epsiode path
    (reset origin back to true again)
    - saves data into new sub folder for now -> later saves it into hierarchical data structure 
    """
    ...
    # initial_Tcp = env.rtde_r.getActualTCPPose()
    ...
    # os.makedirs(episode_name, exist_ok=True)  # This creates the folder if it doesn't exist
    

    ### SAVE DATA INITIALIZATION ### 
    if save_data:
        #Below will be initlizeing with data collection OUTPUT DIRECTORY,metadata? if this has h5py, image name? 
        # episode_name = os.path.join(save_filepath, f"episode_{num_eps}")
        traj_writer = TrajectoryWriter(episode_name, save_images=save_images)        
        #saving interval rounded down, idealy a desion is made to pass in a saving frequency that is a mutiple of Telop env.control_hz (500)...realistcally don't know if it actually goes that fast
    ...
    while True:            
        ...
        controller_info = {} if (controller is None) else controller.get_info() #get_info can be adjusted to return ACTION...MAYBE?
        skip_action = (not controller_info["movement_enabled"]) #true if side button not held down 
        ...
        #     print('comp time: ', (comp_time/1000))
        ...
        # if step_counter % save_hz == 0:
        #     print(step_counter)


        # Save Data #
        if save_data:
            # Write the current timestep to the episode folder by the specified saving frequency different from the fast 500hz control frequency
            if step_counter % save_step == 0:
                # print(step_counter)
            # if ((time_ms() - start_timer) % (1000/save_hz)) == 0:
                control_timestamps["step_end"] = time_ms()
                obs["timestamps"] = control_timestamps
                #contiune writing the time step to "current episode_folder" wiith all it's data:
                traj_writer.write_timestep(obs, action, task)
        
        # If success or failure is true, reset robot and save data into a new subfolder
            ...
            # if toggle_save:
                ...
                traj_writer.write_episode_info(task, False if controller_info["failure"] else True, num_eps)
                
                # toggle_save False
                #ADD A CLOSE ALL COLLECTION FUNCTION IN THE TrajectoryWriter CLASS:
                traj_writer.close()
                print(f"traj writer close")
                
                #reset should not get recorded in data --CHECK THIS (IE AT END OF AN ACTION IT SHOULD NOT GO BACK TO DEFAULT, ONLY AT BEGINING OF AN ACTION SEQUENCE)

                env, controller = horizon_reset(env,controller, control_hz, right_controller=right_controller)
                print(f"reset env to default")
                
                num_eps += 1

                # REASSIGN current episode file for the next epsiode 
                if num_eps < eps_horizon:
                    episode_name = os.path.join(save_filepath, f"episode_{num_eps}.h5")
                    # os.makedirs(episode_name, exist_ok=True) # comment this out to test traj_test_collect

                    # # Save data into this new folder
                    traj_writer = TrajectoryWriter(episode_name, save_images=save_images)
                    # traj_writer.write_timestep({"observation": obs, "action": controller_action_info})
                    #traj_writer.write_timestep(obs, action, task)
        ...
        if cv2.waitKey(1) == 27 or end_traj:  # Exit on 'Esc' key or trajectory end
