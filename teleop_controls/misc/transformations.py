...
#Additional conversion from Rmat to axis_angle in radians and vis-versa 
...
def Euler2Axis_Pose(euler_pose,degrees=False):
    axis_angle = euler_to_axis(euler_pose[3:])
    result = np.concatenate([euler_pose[:3],axis_angle])
    return result

def axis_to_quat(axis, degrees=False):
    quat =  R.from_rotvec(axis, degrees=degrees).as_quat()
    return quat

def quat_to_axis(quat, degrees=False):
    axis =  R.from_quat(quat, degrees=degrees).as_rotvec()
    return axis
### Subtractions ###
